...
class Logic {
    ...
    Logic(Context context, History history, CalculatorDisplay display) {
        ...
        sp = mContext.getSharedPreferences("historySp", Context.MODE_PRIVATE);
        ...
        try {
            ...
            if (!text.equals(result)) {
                ...
                if (mResult.contains("∞")) {
                    ...
                    String expr = mDisplay.getText().toString().trim();
                    if(!expr.equals(""))
                        saveRecord(mResult, expr);
                }
    ...
    private void saveRecord(String result, String expr) {
        Log.d("Calculator", "saverecord");
        int size = sp.getInt("size", 0);
        
        editor.putInt("size", ++size);
        editor.putString("index:" + size, result);
        editor.putString(result, expr);
        editor.commit();
    }
